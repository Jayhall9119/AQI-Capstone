...
# Get summary statistics of the data set.
...
# Unhappy with the above examples I researched for a more interactive option.


# The package plotly gives a more interactive experience.
install.packages("plotly")
library(plotly)

# Create an interactive scatter plot with colored dots for each country
interactive_plot <- ggplot(pivot_table, aes(x = Population_2022, y = Average_AQI, color = Country)) +
  geom_point() +
  labs(title = "Population vs. Average AQI by Country", x = "Population", y = "Average AQI") +
  scale_x_continuous(labels = scales::comma)  # Use comma format for x-axis labels
...
# I am happy with this plot graph but, I would like to have a map visualization.
...
# Export data frame for future analysis.
write.csv(Clean_AQI_Sorted, "C:\\Users\\guaji\\Desktop\\School\\Capstone\\Clean_AQI_Sorted.csv", row.names = FALSE)